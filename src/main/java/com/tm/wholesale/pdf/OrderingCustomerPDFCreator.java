...
/** 
* Generates Ordering Customer PDF
* 
* @author DONG CHEN
*/ 
public class OrderingCustomerPDFCreator extends ITextUtils {
	...
	public OrderingCustomerPDFCreator(){}
	
	public OrderingCustomerPDFCreator(Order order){
		this.setOrder(order);
	}
...
//      PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(inputFile));
		...
		String outputFile = TMUtils.createPath("wholesale" + File.separator
				...
				+ File.separator + "ordering_form_customer_" + this.getOrder().getId()
				+ ".pdf");
...
//        addPDFTitle(orderPDFTitleTable, "CONFIRMATION", ITextFont.arial_colored_bold_23, 22F, PdfPCell.BOTTOM, PdfPCell.ALIGN_RIGHT, 14);
    ...
    public PdfPTable createOrderDetailTable(){
        ...
        if(this.getCodPlans().size()>0){
            ...
            for (OrderDetail cod : this.getCodPlans()) {
            	...
            	if(cod.getPrice_enduser()==null){
                	cod.setPrice_enduser(0d);
            	}
            	if(cod.getUnit()==null){
                	cod.setUnit(0);
            	}
                ...
                BigDecimal price = new BigDecimal(cod.getPrice_enduser());
                ...
                addCol(orderDetailTable, String.valueOf(TMUtils.fillDecimalPeriod(String.valueOf(cod.getPrice_enduser()))), 1, 0F, ITextFont.arial_normal_8, contentPaddingTop, contentPaddingBottom, PdfPCell.ALIGN_RIGHT);
        ...
        if(this.getCodAddOns().size()>0){
            for (OrderDetail cod : this.getCodAddOns()) {
                ...
                addColBottomBorder(orderDetailTable, String.valueOf(TMUtils.fillDecimalPeriod(String.valueOf(cod.getPrice_enduser()))), 1, 0F, ITextFont.arial_normal_8, contentPaddingTop, contentPaddingBottom, PdfPCell.ALIGN_CENTER, borderColor);
    ...
    public PdfPTable createOrderDetailDicountTable(){
        ...
        for (OrderDetail disDetail : this.codDiscounts) {
        	Double subDiscountPrice = (double) (disDetail.getPrice_enduser().intValue()*disDetail.getUnit());
            ...
            addCol(orderDetailTable, TMUtils.fillDecimalPeriod(disDetail.getPrice_enduser().intValue())).font(ITextFont.arial_bold_8).border("b", 1F).fixedHeight(16F).alignH("r").o();
